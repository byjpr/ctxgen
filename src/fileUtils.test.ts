...
import { readContextFiles, writeOutputFile } from "./fileUtils";
...
test("readContextFiles throws error for non-existent file", async () => {
  ...
  await expect(readContextFiles(fileNames)).rejects.toThrow(
...
test("writeOutputFile handles errors", async () => {
  ...
  await expect(writeOutputFile(fileName, content)).rejects.toThrow(
    "Write error"
  );
});
